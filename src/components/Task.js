...
export default class Task extends Component {
  ...
  render() {
    ...
    return (
      <TouchableOpacity style={{borderColor:'#6fa1e2',borderTopWidth:2,height:60,justifyContent:'center'}} onLongPress={this.toggleModal}>
       <Pressable
        style={{
          zIndex:1,
          position:'absolute',
    ...
    onPress={() => { console.log(this.determineColor())
        ...
        <Text style={{paddingTop:7,height:30,fontSize:20, fontWeight: 'bold', color:'#72bbf8', paddingLeft:10}}>{this.props.taskData.taskName}</Text>
        ...
        {this.props.taskData.start != undefined && (
          <Text style={{fontSize:30, fontWeight: 'bold', paddingLeft:10,color:'#8194a4', paddingBottom:10}}>
            {formatHoursMinutes(this.props.taskData.start)} - {formatHoursMinutes(this.props.taskData.end)}
          </Text>
        )}
